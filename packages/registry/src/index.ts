...
// claim, and telemetry. v0.0.1 ships an in-memory store; production swaps
// in Supabase.
...
const store: RegistryStore = new MemoryStore();
const app = new Hono();
...
app.post("/v1/search", async (c) => {
  ...
  if (!parsed.success) {
    return c.json({ error: "Invalid search request", issues: parsed.error.issues }, 400);
  }
...
app.post("/v1/listings", async (c) => {
  ...
  if (!parsed.success) {
    return c.json({ error: "Invalid listing", issues: parsed.error.issues }, 400);
  }
...
app.post("/v1/rate", async (c) => {
  ...
  if (!parsed.success) {
    return c.json({ error: "Invalid rate request", issues: parsed.error.issues }, 400);
  }
...
app.post("/v1/claim", async (c) => {
  ...
  if (!parsed.success) {
    return c.json({ error: "Invalid claim request", issues: parsed.error.issues }, 400);
  }
...
app.post("/v1/claim/verify", async (c) => {
  ...
  if (!parsed.success) {
    return c.json({ error: "Invalid verify request", issues: parsed.error.issues }, 400);
  }
...
const TelemetrySchema = z.object({
  ...
  agent_id: z.string().regex(/^0x[a-fA-F0-9]{40}$/).nullable(),
...
app.post("/telemetry", async (c) => {
  ...
  await store.recordTelemetry(parsed.data as Parameters<RegistryStore["recordTelemetry"]>[0]);
  return c.body(null, 204);
});
